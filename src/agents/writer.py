...
[前言：300-400字。第一句給最重要的事實，然後解釋背景、為何重要、讀者需要知道什麼]
...
## [主題一：技術細節]
[從研究報告取出技術原理，解釋運作方式、技術限制、與現有方案的差異，至少 300 字，有來源引用]

## [主題二：產業與市場影響]
[具體影響分析：受益者、受威脅者、市場規模或估計數字，至少 250 字，點名具體公司或產品]

## 不同聲音
[批評、疑慮、技術限制、倫理爭議。必須並陳多方觀點，至少 150 字，不能只有正面]

## 對台灣的意義
[具體說明對台灣科技業、企業、用戶的影響，150-200 字，給出可操作的建議或觀察]

## 參考來源
[列出研究報告提供的所有來源 URL，每條一行]
```
...
def writer_node(state: NewsState) -> NewsState:
    ...
    """
    # 主要撰稿用 gpt-4o；humanize/edit 用 gpt-4o-mini（省 token）
    llm = ChatOpenAI(
    ...
    # Step 1：article-writing skill 撰稿（gpt-4o）
    messages = [
    ...
    # Step 2：Humanizer - 去除 AI 寫作痕跡（gpt-4o-mini）
    humanized = _humanize(llm_cheap, draft)

    # Step 3：Editor - 語言精簡與流暢度（gpt-4o-mini）
    edited = _edit(llm_cheap, humanized)
